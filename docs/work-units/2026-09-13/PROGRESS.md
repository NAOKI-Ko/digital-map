...
| WU-10 | KAN-42 | PASS | PENDING COMMIT | BOM CSV template uses stable semantic/definition keys from current enabled fields. Preview reports row totals, errors and nonblocking duplicates; required/type/category/structure errors block a single all-or-nothing transaction. Imports are new-only, one-Floor, unpositioned and unpublished. Full 33 files / 260 tests and typecheck PASS. |
