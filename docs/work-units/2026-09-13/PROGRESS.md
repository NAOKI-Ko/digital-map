...
| WU-07 | KAN-50 | PASS | `4ac90b5` | MAP-scoped standard/custom field definitions, six defaults, address/website columns, typed custom values, mutation/required/type/deletion rules, and admin configuration UI. Full 31 files / 249 tests, typecheck, Prisma validate, and build PASS. Migration preflight remains unavailable without DATABASE_URL and exits fail-closed with code 2. |
| WU-08 | KAN-50 | PASS | `f453eca` | Admin form renders enabled definitions while preserving disabled stored values. Public detail emits only enabled/public/nonblank fields in definition order, separates description and website action, and supports ordered photos. Full 31 files / 251 tests, typecheck and build PASS. |
| WU-09 | KAN-38 | PASS | `bb537c0` | Existing Spot addresses can produce selectable geocoder candidates only on georeferenced Floors; candidates convert to temporary IMAGE x/y, reject out-of-Floor results, remain adjustable, and persist only through the explicit position save. Full 32 files / 254 tests, typecheck and build PASS. |
| WU-10 | KAN-42 | PASS | `8084e64` | BOM CSV template uses stable semantic/definition keys from current enabled fields. Preview reports row totals, errors and nonblocking duplicates; required/type/category/structure errors block a single all-or-nothing transaction. Imports are new-only, one-Floor, unpositioned and unpublished. Full 33 files / 260 tests, typecheck and build PASS. |
| WU-11 | KAN-44/37 | PASS | `4813514` | Creation begins with enabled Illustration versus visibly disabled future Real Map, accepts name and stable public slug, creates default Spot fields, and enters a setup checklist. Tourism categories are opt-in alongside custom names; georeference and branding remain optional. No GEO persistence added. Full 34 files / 265 tests, typecheck and build PASS. |
| WU-12 | KAN-45 | PASS | `1a8f4d1` | FloorDecoration uses MediaAsset plus normalized x/y/width, rotation and layer order. Admin supports library/upload, drag, aspect-preserving resize, rotate, reorder, duplicate and instance-only delete. Public rendering is noninteractive raster below DOM PIN markers; georeference is only a render transform. Full 35 files / 269 tests, typecheck, Prisma generate and build PASS. |
...
| WU-14 | KAN-26/27/43 | PASS | PENDING COMMIT | Category management shows usage count and a filtered Spot-list link. Bulk Category controls appear only with a selection, operate on exactly one Category with confirmation, and use idempotent add/remove that preserves unrelated relations and rejects cross-Map IDs. Full 35 files / 271 tests and typecheck PASS. |
