...
1. Set `DATABASE_URL`, `NUXT_UPLOAD_DIR` (or `MEDIA_ROOT`), and `BACKUP_ROOT` without printing them.
2. Run `pnpm backup:db` and `pnpm backup:media`.
3. Run `pnpm backup:verify -- <dump-path> <media-backup-directory>`.
...
Use only a new disposable PostgreSQL database and temporary media directory. Set `RESTORE_DATABASE_URL`, `RESTORE_MEDIA_ROOT`, and `ALLOW_DISPOSABLE_RESTORE=true`; the DB wrapper refuses equality with `DATABASE_URL`. Restore DB and media, run Prisma/data checks, verify every managed `MediaAsset.storageKey`, then remove the disposable resources. Never use QA, production, or a shared database for a drill.
