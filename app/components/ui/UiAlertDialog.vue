...
<template>
  <AlertDialogRoot :open="open" @update:open="value => { if (!value && !busy) emit('cancel') }">
    <AlertDialogPortal>
      ...
      <AlertDialogContent class="fixed left-1/2 top-1/2 z-[71] max-h-[calc(100svh-2rem)] w-[calc(100vw-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl outline-none sm:p-6">
        ...
        <div class="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <AlertDialogCancel as-child>
            <button type="button" :disabled="busy" class="rounded-lg border border-stone-300 px-4 py-2.5 text-sm font-semibold text-stone-700 disabled:opacity-50" @click="emit('cancel')">{{ cancelLabel }}</button>
          </AlertDialogCancel>
          ...
          <AlertDialogAction as-child>
            <button type="button" :disabled="busy" class="rounded-lg px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50" :class="destructive ? 'bg-red-700' : 'bg-terracotta-600'" @click="emit('confirm')">{{ busy ? '処理中…' : confirmLabel }}</button>
          </AlertDialogAction>
        </div>
      </AlertDialogContent>
    </AlertDialogPortal>
