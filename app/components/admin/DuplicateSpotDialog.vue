...
<template>
  <AppDialog :open="open" title="同名のスポットがあります" description="重複登録は可能です。既存スポットを確認して、このまま保存するか選んでください。" @close="$emit('cancel')">
    <ul class="divide-y rounded-lg border"><li v-for="match in matches" :key="match.id" class="p-3 text-sm"><b>{{ match.name }}</b><p class="mt-1 text-stone-600">{{ match.floorName }} · {{ match.positioned ? '配置済み' : '位置未設定' }} · {{ match.categoryNames.join(' / ') || 'カテゴリーなし' }}</p></li></ul>
