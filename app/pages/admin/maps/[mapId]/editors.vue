...
async function addEditor() {
  ...
  await run(async () => {
    ...
    await refresh()
    success('編集者を追加しました', 'map-editor-operation')
  })
}
async function removeEditor(userId: string) {
  await run(async () => {
    ...
    await refresh()
    success('編集者を解除しました', 'map-editor-operation')
  })
}
  ...
  <div class="max-w-4xl">
    ...
    <section v-else class="mt-6 border-t border-stone-200 pt-5">
      ...
      <form class="mt-5 flex gap-3" @submit.prevent="addEditor">
        <UiSelect v-model="selectedUserId" class="min-w-0 flex-1" label="追加するワークスペースメンバー" :options="[{ value: '', label: 'ワークスペースのメンバーを選択' }, ...(data?.candidates ?? []).map(candidate => ({ value: candidate.userId, label: candidate.displayName || candidate.email }))]" />
