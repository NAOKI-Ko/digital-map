...
<template>
  <div class="max-w-5xl">
    ...
    <header class="mt-5">
      ...
      <h1 class="mt-1 text-3xl font-bold text-stone-900">CSVでまとめて編集</h1>
      <p class="mt-2 text-sm text-stone-600">1つのフロアの既存スポットをCSVに書き出して安全にまとめて編集できます。従来形式のCSVは新規登録専用として引き続き利用できます。</p>
    </header>
    <section class="mt-8 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
      <div class="flex flex-wrap gap-3">
        ...
        <a :href="exportUrl || undefined" :aria-disabled="!floorId" :class="!floorId ? 'pointer-events-none opacity-40' : ''" class="inline-flex rounded-lg border border-stone-300 px-4 py-2.5 text-sm font-semibold">既存スポットをCSVで書き出す</a>
      </div>
    ...
    <section v-if="preview" class="mt-6 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
      ...
      <ul class="mt-5 divide-y divide-stone-200 border-y border-stone-200">
        <li v-for="row in preview.rows" :key="row.rowNumber" class="py-3 text-sm">
          <div class="flex flex-wrap items-center gap-2"><b>{{ row.rowNumber }}行目: {{ row.name || '（名前なし）' }}</b><span v-for="classification in row.classifications" :key="classification" class="rounded bg-stone-100 px-2 py-0.5 text-xs font-bold">{{ classificationLabel(classification) }}</span></div>
