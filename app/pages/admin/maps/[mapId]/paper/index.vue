...
<template>
  <div class="max-w-6xl">
    ...
    <section class="mt-7 rounded-2xl border border-terracotta-200 bg-terracotta-50 p-5"><h2 class="font-bold text-stone-900">まずはおすすめ設定で作成</h2><p class="mt-1 text-sm text-stone-700">現在の公開可能なスポット {{ data?.source.spotCount ?? 0 }}件から、自動で読みやすい構成を提案します。</p><NuxtLink :to="`/admin/maps/${mapId}/paper/new`" class="mt-4 inline-block text-sm font-bold text-terracotta-700">用途を選んで始める →</NuxtLink></section>
    ...
    <section v-else class="mt-8"><div class="flex items-center justify-between"><h2 class="text-lg font-bold">保存した紙マップ</h2><NuxtLink :to="`/admin/maps/${mapId}/paper/request`" class="text-sm font-semibold text-stone-600 hover:text-stone-900">デザイン制作を相談する</NuxtLink></div>
