...
async function main() {
  ...
  if (process.env.DATABASE_URL) {
    if (canonicalDatabaseTarget(process.env.DATABASE_URL) === canonicalDatabaseTarget(targetValue)) throw new Error('Refusing to restore into DATABASE_URL')
    if (databaseName(process.env.DATABASE_URL) === databaseName(targetValue)) throw new Error('Disposable restore database name must differ from DATABASE_URL')
  }
